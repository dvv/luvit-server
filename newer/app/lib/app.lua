...
function Application.prototype:set(option, value)
  ...
  if self.options[option] ~= value then
    ...
    self:debug('SET OPTION', option, value)
  end
  return self
end
...
function Application.prototype:log(level, fmt, ...)
  -- choose color
...
function Application.prototype:info(fmt, ...)
  self:log('info', fmt, ...)
  return self
end

function Application.prototype:error(fmt, ...)
  self:log('error', fmt, ...)
  return self
end

function Application.prototype:debug(fmt, ...)
...
function Application.prototype:use(handler, ...)
  ...
  if type(handler) == 'string' then
    local plugin = require(Path.join(__dirname, handler))
    layer = plugin(...)
  else
    layer = handler
  end
...
function Application.prototype:run(port, host)
  ...
  self:info('Server listening at http://%s:%d/. Press CTRL+C to stop...', host, port)
  return self
end
